...
public static class AuthorizationMessagePipelineExtensions
{
    public static IMessagePipeline<TMessage, TResponse> UseAuthorization<TMessage, TResponse>(
        ...
        where TMessage : class, IMessage<TMessage, TResponse> =>
        pipeline.Use(new MessageAuthorizationMiddleware<TMessage, TResponse>());

    public static IMessagePipeline<TMessage, TResponse> RequirePermission<TMessage, TResponse>(
        this IMessagePipeline<TMessage, TResponse> pipeline,
        string permission
    )
        where TMessage : class, IMessage<TMessage, TResponse>
    {
        return pipeline.Configure<MessageAuthorizationMiddleware<TMessage, TResponse>>(m =>
            m.Configuration.Permission = permission
        );
    }
}
