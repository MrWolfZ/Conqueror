...
public static class DefaultPipelineExtensions
{
    ...
    {
        return pipeline.UseLogging().WithIndentedJsonPayloadLogFormatting();
    }
                ...
                p.UseLogging(c =>
                {
                    if (loggerCategoryType is not null)
                    {
                        c.LoggerCategoryFactory = _ =>
                            loggerCategoryType.FullName ?? loggerCategoryType.Name;
                    }
                })
        );
    ...
    public static TIHandler WithDefaultSenderPipeline<TMessage, TResponse, TIHandler>(
        ...
        handler.WithPipeline(p => p.UseDefaultForSender(loggerCategoryType));
