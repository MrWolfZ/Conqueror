...
public static class ConquerorContextStreamingExtensions
{
    ...
    /// <returns>the ID of the executing streaming request if there is one, otherwise <see langword="null" /></returns>
    public static string? GetStreamingRequestId(this ConquerorContext conquerorContext) =>
        conquerorContext.TransportableData.Get(StreamingRequestIdKey);
