...
{
    private IApiDescriptionGroupCollectionProvider ApiDescriptionProvider =>
        Resolve<IApiDescriptionGroupCollectionProvider>();
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptors()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequest).FullName,
        ...
        Assert.That(
            queryApiDescription?.SupportedResponseTypes.Select(t => t.StatusCode).Single(),
            Is.EqualTo(expected: 200)
        );
        Assert.That(queryApiDescription?.GroupName, Is.Null);
        Assert.That(queryApiDescription?.ParameterDescriptions.Count, Is.EqualTo(expected: 0));
    }
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithoutPayload()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequestWithoutPayload).FullName,
                StringComparison.Ordinal
            )
        );
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithComplexPayload()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequestWithComplexPayload).FullName,
                StringComparison.Ordinal
            )
        );
        ...
        Assert.That(queryApiDescription?.HttpMethod, Is.EqualTo(HttpMethods.Get));
        Assert.That(
            queryApiDescription?.RelativePath,
            Is.EqualTo("api/streams/TestStreamingRequestWithComplexPayload")
        );
        Assert.That(
            queryApiDescription?.SupportedResponseTypes.Select(t => t.StatusCode).Single(),
            Is.EqualTo(expected: 200)
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithCustomPathConvention()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequest3).FullName,
                StringComparison.Ordinal
            )
        );
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithCustomPath()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequestWithCustomPath).FullName,
                StringComparison.Ordinal
            )
        );
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithVersion()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequestWithVersion).FullName,
                StringComparison.Ordinal
            )
        );
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithOperationId()
    {
        var apiDescriptions = ApiDescriptionProvider.ApiDescriptionGroups.Items.SelectMany(i => i.Items);
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(d.ActionDescriptor.AttributeRouteInfo?.Name, "custom-stream-op-id", StringComparison.Ordinal)
        );
    ...
    public void ApiDescriptionProvider_ReturnsStreamDescriptorsWithApiGroupName()
    {
        ...
        var queryApiDescription = apiDescriptions.FirstOrDefault(d =>
            string.Equals(
                d.ActionDescriptor.AttributeRouteInfo?.Name,
                typeof(TestStreamingRequestWithApiGroupName).FullName,
                StringComparison.Ordinal
            )
        );
    ...
    protected override void ConfigureServices(IServiceCollection services)
    {
        _ = services
            ...
            {
                o.PathConvention = new TestHttpStreamPathConvention();
            });
    ...
    public sealed class TestStreamingRequestWithOperationIdHandler
        : IStreamProducer<TestStreamingRequestWithOperationId, TestItem>
    {
        public async IAsyncEnumerable<TestItem> ExecuteRequest(
            TestStreamingRequestWithOperationId request,
            [EnumeratorCancellation] CancellationToken cancellationToken = default
        )
        {
            await Task.Yield();
    ...
    public sealed class TestStreamingRequestWithApiGroupNameHandler
        : IStreamProducer<TestStreamingRequestWithApiGroupName, TestItem>
    {
        public async IAsyncEnumerable<TestItem> ExecuteRequest(
            TestStreamingRequestWithApiGroupName request,
            [EnumeratorCancellation] CancellationToken cancellationToken = default
        )
        {
            await Task.Yield();
