...
internal sealed class ConfigurationProvider(
    IEnumerable<Action<ConquerorStreamingHttpClientGlobalOptions>> configureGlobalOptions
)
{
    private readonly IReadOnlyCollection<Action<ConquerorStreamingHttpClientGlobalOptions>> configureGlobalOptions =
        configureGlobalOptions.ToList();

    private readonly ConcurrentDictionary<Type, bool> httpValidityByRequestType = [];
    ...
    public ResolvedHttpClientOptions GetOptions(IServiceProvider provider, HttpClientRegistration registration)
    {
        ...
        if (!baseAddress.IsAbsoluteUri)
        {
            throw new InvalidOperationException(
                $"configuration error while creating options for Conqueror HTTP transport client: base address must be an absolute URI, but got '{baseAddress}'"
            );
        }
        ...
        return new ResolvedHttpClientOptions(
            socketFactory,
            baseAddress,
    ...
    private ConquerorStreamingWebSocketFactory CreateWebSocket(
        HttpClientRegistration registration,
        ConquerorStreamingHttpClientGlobalOptions globalOptions
    )
    {
        if (
            registration.RequestType is not null
            && (
                globalOptions.StreamWebSocketFactories?.TryGetValue(
                    registration.RequestType,
                    out var queryClientFactory
                ) ?? false
            )
        )
        {
        ...
        if (
            ...
            && (
                globalOptions.AssemblyWebSocketFactories?.TryGetValue(
                    requestType.Assembly,
                    out var assemblyClientFactory
                ) ?? false
            )
        )
        {
    ...
    private async Task<WebSocket> DefaultWebSocketFactory(
        Uri uri,
    ...
    private void ValidateRequestType(HttpClientRegistration registration)
    {
        ...
        {
            var isHttpQuery = httpValidityByRequestType.GetOrAdd(
                requestType,
                t => t.GetCustomAttribute<HttpStreamAttribute>() is not null
            );

            if (!isHttpQuery)
            {
                throw new InvalidOperationException(
                    $"streaming request type '{requestType.Name}' is marked as an HTTP stream; did you forget to add the '[{nameof(HttpStreamAttribute)}]'?"
                );
            }
        }
    }
