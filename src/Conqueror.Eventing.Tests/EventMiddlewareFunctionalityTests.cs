...
public sealed class EventMiddlewareFunctionalityTests
{
    ...
    public async Task GivenPublishAndObserverPipelines_WhenObserverIsCalled_MiddlewaresAreCalledWithEvent(ConquerorMiddlewareFunctionalityTestCase testCase)
    {
        ...
        var expectedTransportTypes = testCase.ExpectedMiddlewareTypes.Select(t => new EventTransportType(InProcessEventTransportTypeExtensions.TransportName, t.TransportRole)).ToList();
        var expectedTransportTypesFromPipelineBuilders = testCase.ExpectedTransportRolesFromPipelineBuilders.Select(r => new EventTransportType(InProcessEventTransportTypeExtensions.TransportName, r)).ToList();
    ...
    public async Task GivenObserverAndPublishPipeline_WhenObserverIsCalled_TransportTypesInPipelinesAreCorrect()
    {
        ...
        Assert.That(transportTypeFromPublish, Is.EqualTo(new EventTransportType(InProcessEventTransportTypeExtensions.TransportName, EventTransportRole.Publisher)));
        Assert.That(transportTypeFromObserver, Is.EqualTo(new EventTransportType(InProcessEventTransportTypeExtensions.TransportName, EventTransportRole.Receiver)));
    ...
    public async Task GivenEventTypeWithCustomTransport_WhenObserversAreCalledFromBroadcaster_MiddlewaresAreCalledWithEventAndCancellationToken()
    {
        ...
        var broadcaster = provider.GetRequiredService<IConquerorEventTransportReceiverBroadcaster>();
    ...
    private sealed class TestEventTransportPublisher(TestObservations observations, Exception1? exceptionToThrow = null)
        : IConquerorEventTransportPublisher<TestEventTransportAttribute>
    {
    ...
    private sealed class TestEventTransportPublisher2(TestObservations observations, Exception2? exceptionToThrow = null) : IConquerorEventTransportPublisher<TestEventTransport2Attribute>
    {
