...
public sealed class EventTransportPublisherRegistrationTests
{
    ...
    private sealed class TestEventTransportAttribute() : ConquerorEventTransportAttribute(nameof(TestEventTransportAttribute));

    [AttributeUsage(AttributeTargets.Class)]
    private sealed class TestEventTransport2Attribute() : ConquerorEventTransportAttribute(nameof(TestEventTransport2Attribute));

    [AttributeUsage(AttributeTargets.Class)]
    public sealed class TestEventTransportForAssemblyScanningAttribute() : ConquerorEventTransportAttribute(nameof(TestEventTransportForAssemblyScanningAttribute));

    private sealed class TestEventTransportPublisher : IConquerorEventTransportPublisher<TestEventTransportAttribute>
    {
        public Task PublishEvent<TEvent>(TEvent evt, TestEventTransportAttribute configurationAttribute, IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
            where TEvent : class
        {
    ...
    private sealed class MultiTestEventTransportPublisher : IConquerorEventTransportPublisher<TestEventTransportAttribute>,
                                                            IConquerorEventTransportPublisher<TestEventTransport2Attribute>
    {
        public Task PublishEvent<TEvent>(TEvent evt, TestEventTransportAttribute configurationAttribute, IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
            where TEvent : class
        {
        ...
        public Task PublishEvent<TEvent>(TEvent evt, TestEventTransport2Attribute configurationAttribute, IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
            where TEvent : class
        {
            throw new NotSupportedException();
        }
    ...
    public sealed class TestEventTransportForAssemblyScanningPublisher : IConquerorEventTransportPublisher<TestEventTransportForAssemblyScanningAttribute>
    {
        public Task PublishEvent<TEvent>(TEvent evt, TestEventTransportForAssemblyScanningAttribute configurationAttribute, IServiceProvider serviceProvider, CancellationToken cancellationToken = default)
            where TEvent : class
        {
            throw new NotSupportedException();
        }
