    ...
    where TMessage : class, IFileSystemMessage<TMessage, TResponse>
{
    static virtual string Tag { get; } =
        $"{Dasherize(typeof(TMessage).Name.EndsWith("Message", StringComparison.Ordinal) ? typeof(TMessage).Name[..^7] : typeof(TMessage).Name)}";
    ...
    internal static virtual IFileSystemMessageResponseSerializer<
        TMessage,
        TResponse
    > FileSystemMessageResponseSerializer => FileSystemMessageResponseJsonSerializer<TMessage, TResponse>.Default;
    ...
    private static string Dasherize(string str)
    {
        ...
        {
            _ = char.IsUpper(str[i])
                ? sb.Append(value: '-').Append(char.ToLower(str[i], CultureInfo.InvariantCulture))
                : sb.Append(str[i]);
        }
    ...
    [SuppressMessage(
        ...
        Justification = "we expect lower-case to not be a problem"
    )]
