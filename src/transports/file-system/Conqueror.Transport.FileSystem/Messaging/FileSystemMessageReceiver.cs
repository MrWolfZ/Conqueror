...
) : IFileSystemMessageReceiver
{
    private readonly Dictionary<
        string,
        (string MessageFileExtension, string ResponseFileExtension)
    > fileExtensionsByTag = [];
    ...
    public FileSystemMessageReceiverConfiguration EnableSingleInstance(
        string baseDirectoryPath,
        TimeSpan pollingInterval
    )
    {
        Configuration = new FileSystemMessageReceiverConfiguration
        {
    ...
    public FileSystemMessageReceiverConfiguration EnableMultipleCompetingInstances(
        string baseDirectoryPath,
        ...
        where TMessage : class, IFileSystemMessage<TMessage, TResponse>
    {
        ...
        invokerByTag[TMessage.Tag] = async (message, ct) =>
        {
            var response = await invoker
                .Invoke<TMessage, TResponse>((TMessage)message, ServiceProvider, TransportName, ct)
        ...
        parserByTag[TMessage.Tag] = async (content, ct) =>
            await TMessage
                .FileSystemMessageSerializer.DeserializeMessage(ServiceProvider, content, ct)
                .ConfigureAwait(false);

        responseWriterByTag[TMessage.Tag] = async (response, content, ct) =>
            await TMessage
                .FileSystemMessageResponseSerializer.SerializeResponse(
                    ServiceProvider,
                    (TResponse)response,
                    content,
                    ct
    ...
    public Task WriteResponse(string tag, object response, Stream stream, CancellationToken cancellationToken) =>
        responseWriterByTag[tag].Invoke(response, stream, cancellationToken);

    public Task<object> InvokeHandler(string tag, object message, CancellationToken cancellationToken) =>
        invokerByTag[tag].Invoke(message, cancellationToken);
}
