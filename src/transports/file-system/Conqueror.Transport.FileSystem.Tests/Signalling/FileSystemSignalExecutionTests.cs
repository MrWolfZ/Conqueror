using static Conqueror.Transport.FileSystem.Tests.Signalling.FileSystemSignalTestCases;
...
public sealed partial class FileSystemSignalExecutionTests
{
    ...
    [Test]
    [Combinatorial]
    public void GivenMultipleFileSystemSignalTypesWithSameTag_WhenRunningReceivers_ThrowsException([Values] bool runIndividually)
    {
        var clientServices = new ServiceCollection().AddConquerorFileSystemTransport()
        ...
        Assert.That(
            () => runIndividually
                ? signalReceivers.RunFileSystemSignalReceiver<TestSignalWithDuplicateTagHandler>(CancellationToken.None)
    ...
    public async Task GivenMultipleCompetingHandlers_OneHandlerFailsProcessing_TheOtherHandlerProcessesTheSignal()
    {
        ...
        await using var handle1 = host1.Resolve<ISignalReceivers>().RunFileSystemSignalReceivers(testTimeouts.TestTimeoutToken);
        await using var handle2 = host2.Resolve<ISignalReceivers>().RunFileSystemSignalReceivers(testTimeouts.TestTimeoutToken);
        ...
        var publisher = senderServiceProvider.GetRequiredService<ISignalPublishers>()
                                             .For(TestSignal.T)
