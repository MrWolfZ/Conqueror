...
public sealed class HttpSignalTransportConformityReceiverTestHost : ISignalTransportConformityReceiverTestHost
{
    ...
    private readonly Action<HttpSignalTransportConformityReceiverTestHost> onDispose;

    private HttpSignalTransportConformityReceiverTestHost(
        ServiceProvider serviceProvider,
        Action<HttpSignalTransportConformityReceiverTestHost> onDispose)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        _ = services.AddConquerorHttpClient()
                    .AddSingleton<Action<IHttpSseSignalReceiver>>(r => ConfigureSseReceiver(
                                                                      host,
                                                                      testCase,
                                                                      r,
                                                                      reconnectDelayCallback))
                    .AddSingleton<Action<IHttpWebSocketsSignalReceiver>>(r => ConfigureWebSocketsReceiver(
                                                                             host,
                                                                             testCase,
                                                                             r,
                                                                             reconnectDelayCallback))
        ...
        var receiverHost = new HttpSignalTransportConformityReceiverTestHost(serviceProvider, h =>
        {
        ...
        reg = cancellationToken.Register(h => onDisposeOrCancel((HttpSignalTransportConformityReceiverTestHost)h!), receiverHost);
        ...
        Func<CancellationToken, Task>? reconnectDelayCallback)
    {
        ...
        var config = receiver.Enable(HttpSignalTransportConformityTestHost.SseAddress)
        ...
        if (reconnectDelayCallback != null)
        {
        ...
        Func<CancellationToken, Task>? reconnectDelayCallback)
    {
        ...
        var config = receiver.Enable(HttpSignalTransportConformityTestHost.WebSocketsAddress)
