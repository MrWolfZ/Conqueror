...
public sealed class HttpMessageApiDescriptionTests
{
        ...
        Type? responseType)
    {
        ...
        Assert.That(messageApiDescription.SupportedResponseTypes.Select(t => t.Type), Is.EquivalentTo(new[] { responseType ?? typeof(void) }));
        ...
        Type? responseType)
    {
        ...
        var swaggerContent = await receiverHost.HttpClient.GetStringAsync("/swagger/v1/swagger.json");
        ...
        Assert.That(
            ...
            testCase.MessageContentType is null ? Is.Null.Or.Empty : Is.EquivalentTo(new[] { testCase.MessageContentType }));
        ...
        if (testCase.ResponseContentType is not null)
        {
            ...
            if (responseType is not null && responseType.IsAssignableTo(typeof(IEnumerable)))
            {
