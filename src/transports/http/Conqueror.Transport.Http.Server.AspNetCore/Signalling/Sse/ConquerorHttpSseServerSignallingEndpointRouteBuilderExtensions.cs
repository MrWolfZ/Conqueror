...
public static class ConquerorHttpSseServerSignallingEndpointRouteBuilderExtensions
{
    ...
    public static IEndpointConventionBuilder MapServerSentEventsSignalsEndpoint(
    ...
    )
    {
        return builder.MapGet(
            ...
            async context =>
            {
                ...
                if (eventTypes.Count is 0)
                {
                    context.Response.StatusCode = StatusCodes.Status400BadRequest;
                    context.Response.Headers.Append(HeaderNames.ContentType, ContentTypes.TextPlain);
                    await context
                        .Response.WriteAsync("must provide at least one signal type", context.RequestAborted)
                ...
                await SseEndpoint
                    .Run(
                        context,
                        logger,
                        () => context.RequestServices.GetRequiredService<HttpSseSignalBroker>().Subscribe(eventTypes)
                    )
                    .ConfigureAwait(false);
            }
        );
