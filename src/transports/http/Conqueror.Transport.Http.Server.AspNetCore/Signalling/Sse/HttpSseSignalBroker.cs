...
internal sealed class HttpSseSignalBroker(IServiceProvider serviceProvider)
{
        ...
        where TSignal : class, IHttpSseSignal<TSignal>
    {
        ...
        var envelope = new HttpSseSignalEnvelope(signal, typeof(TSignal), conquerorContext.EncodeDownstreamContextData());
    ...
    {
        var formatter = itemFormatterBySignalType.GetValueOrDefault(item.Data.SignalType);
