...
using Authorization.Messaging;

[TestFixture]
internal sealed partial class AuthorizationMessageMiddlewareTests
{
    [Test]
    public async Task GivenHandlerWithAuthorizationMiddlewareWithoutConfiguration_WhenCallingHandler_CallSucceeds()
    {
        ...
        var handler = host.Resolve<IMessageSenders>().For(TestMessage.T).WithPipeline(p => p.UseAuthorization());
    ...
    )
    {
        ...
        var handler = host.Resolve<IMessageSenders>()
            .For(TestMessage.T)
            .WithPipeline(p =>
                p.UseAuthorization(c =>
                    {
                        _ = authenticationCheck switch
                        {
                            "none" => c,
                            "syncSuccess" => c.AddAuthorizationCheck(authenticationCheckName, ctx => ctx.Success()),
                            "syncFail" => c.AddAuthorizationCheck(
                                authenticationCheckName,
                                ctx => ctx.Unauthenticated("test failure")
                            ),
                            "asyncSuccess" => c.AddAuthorizationCheck(
                                authenticationCheckName,
                                async ctx =>
                                {
                                    await Task.Yield();
                            ...
                            "asyncFail" => c.AddAuthorizationCheck(
                                authenticationCheckName,
                                async ctx =>
                                {
                                    await Task.Yield();
                    ...
                    .ConfigureAuthorization(c =>
                    {
                        _ = authorizationCheck switch
                        {
                            ...
                            "syncFail" => c.AddAuthorizationCheck(
                                authorizationCheckName,
                                ctx => ctx.Unauthorized("test failure")
                            ),
                            "asyncSuccess" => c.AddAuthorizationCheck(
                                authorizationCheckName,
                                async ctx =>
                    ...
                    .ConfigureAuthorization(c =>
                    {
                        var expectedChecks = new List<string>()
                            .Concat(authenticationCheck is not "none" ? [authenticationCheckName] : [])
        ...
        if (
            authenticationCheck is not "syncFail" and not "asyncFail"
            && authorizationCheck is not "syncFail" and not "asyncFail"
        )
        {
            var response = await handler.Handle(new() { Payload = 10 }, host.TestTimeoutToken);
        ...
        var expectedFailureReason =
            string.Equals(authenticationCheck, "syncFail", StringComparison.Ordinal)
            || string.Equals(authenticationCheck, "asyncFail", StringComparison.Ordinal)
        ...
        string[] expectedFailureDetails =
            string.Equals(authenticationCheck, "syncFail", StringComparison.Ordinal)
            || (
                authenticationCheck is not "asyncFail"
                && string.Equals(authorizationCheck, "syncFail", StringComparison.Ordinal)
            )
                ? ["test failure"]
                : ["test failure 1", "test failure 2"];

        await Assert.ThatAsync(
            () => handler.Handle(new() { Payload = 10 }, host.TestTimeoutToken),
            Throws
                .Exception.TypeOf<MessageAuthorizationFailedException>()
                .With.Property(nameof(MessageAuthorizationFailedException.WellKnownReason))
                .EqualTo(expectedFailureReason)
                .And.Matches<MessageAuthorizationFailedException>(e =>
                    e.Result.Details.SequenceEqual(expectedFailureDetails, StringComparer.Ordinal)
                )
        );
    }
    ...
    public async Task GivenHandlerWithAuthorizationMiddlewareWithFailingCheck_WhenRemovingFailingCheckAndCallingHandler_CallSucceeds()
    {
        ...
        var handler = host.Resolve<IMessageSenders>()
            .For(TestMessage.T)
            .WithPipeline(p =>
                p.UseAuthorization(c => c.AddAuthorizationCheck("test", ctx => ctx.Unauthenticated("test")))
                    .ConfigureAuthorization(c =>
                        c.RemoveAuthorizationCheck("test")
    ...
    public async Task GivenHandlerWithAuthorizationMiddlewareWithCheck_WhenCallingHandler_CorrectContextIsPassedToCheck()
    {
        ...
        using var conquerorContext = scope
        ...
        var handler = scope
            .ServiceProvider.GetRequiredService<IMessageSenders>()
            .For(TestMessage.T)
            .WithPipeline(p =>
                p.UseAuthorization(c =>
                    c.AddAuthorizationCheck(
                            "test",
                            ctx =>
                            {
                                seenContext = ctx;
