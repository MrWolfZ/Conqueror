...
[ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(C0006QueryHandlerMustImplementPipelineConfigurationInterfaceAnalyzerCodeFixProvider))]
[Shared]
public sealed class C0006QueryHandlerMustImplementPipelineConfigurationInterfaceAnalyzerCodeFixProvider : CodeFixProvider
{
    public override ImmutableArray<string> FixableDiagnosticIds => ImmutableArray.Create(C0006QueryHandlerMustImplementPipelineConfigurationInterfaceAnalyzer.DiagnosticId);
    ...
    public override async Task RegisterCodeFixesAsync(CodeFixContext context)
    {
        ...
        context.RegisterCodeFix(
            CodeAction.Create(
                Resources.Analyzer0006CodeFixTitle,
                c => ImplementPipelineConfigurationInterface(context.Document, declaration, c),
                nameof(Resources.Analyzer0006CodeFixTitle)),
            diagnostic);
    }
    ...
    private static async Task<Solution> ImplementPipelineConfigurationInterface(Document document, TypeDeclarationSyntax typeDecl, CancellationToken cancellationToken)
    {
        ...
        var baseType = SyntaxFactory.SimpleBaseType(SyntaxFactory.ParseTypeName("Conqueror.IConfigureQueryPipeline"));
