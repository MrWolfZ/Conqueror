...
internal sealed class AggregateSignalPublisher<TSignal>(
    IReadOnlyCollection<ISignalPublisher<TSignal>> publishers)
    : IAggregateSignalPublisher<TSignal>
    where TSignal : class, ISignal<TSignal>
{
    public string TransportTypeName { get; } = $"{ConquerorConstants.AggregateTransportName}[{string.Join(",", publishers.Select(p => p.TransportTypeName))}]";
    ...
    {
        var fns = publishers.Select<ISignalPublisher<TSignal>, SignalHandlerFn<TSignal>>(p => (s, sp, ct) => p.Publish(
                                                                                             s,
                                                                                             sp,
                                                                                             conquerorContext,
                                                                                             ct))
    ...
    public IAggregateSignalPublisher<TSignal> WithSequentialBroadcastingStrategy(Action<SequentialSignalBroadcastingStrategyConfiguration> configure)
    {
    ...
    public IAggregateSignalPublisher<TSignal> WithParallelBroadcastingStrategy(Action<ParallelSignalBroadcastingStrategyConfiguration> configure)
    {
