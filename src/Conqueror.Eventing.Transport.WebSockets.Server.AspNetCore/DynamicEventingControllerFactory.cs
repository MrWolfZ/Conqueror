...
internal static class DynamicEventingControllerFactory
{
    private static readonly AssemblyBuilder DynamicAssembly = AssemblyBuilder.DefineDynamicAssembly(new("ConquerorEventingTransportWebSocketsServerAspNetCoreDynamic"), AssemblyBuilderAccess.Run);
    private static readonly ModuleBuilder ModuleBuilder = DynamicAssembly.DefineDynamicModule("ConquerorEventingTransportWebSocketsServerAspNetCoreDynamicModule");
    ...
    {
        var typeName = $"{name}`ConquerorEventingTransportWebSocketsServerAspNetCoreDynamicController";
    ...
    {
        var ctor = typeof(ConquerorEventingWebSocketsTransportControllerRouteValueAttribute).GetConstructors().First(c => c.GetParameters().Length == 1 && c.GetParameters().Single().ParameterType == typeof(string));
