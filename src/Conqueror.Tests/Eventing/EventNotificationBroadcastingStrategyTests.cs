...
public sealed partial class EventNotificationBroadcastingStrategyTests
{
    ...
    public async Task GivenCustomBroadcastingStrategy_WhenPublishingEvent_CustomStrategyIsUsed()
    {
        ...
        {
            (typeof(TestBroadcastingStrategy), notification),
            (typeof(TestBroadcastingStrategy), notification),
        }));
    ...
    public async Task GivenCustomBroadcastingStrategy_WhenPublishingEvent_StrategyIsResolvedFromPublishingScope()
    {
        ...
        var handler1 = scope1.ServiceProvider.GetRequiredService<IEventNotificationHandler<TestEventNotification>>();

        var handler2 = scope2.ServiceProvider.GetRequiredService<IEventNotificationHandler<TestEventNotification>>();
    ...
    public void GivenCustomBroadcastingStrategy_WhenStrategyThrows_SameExceptionIsRethrownFromPublishing()
    {
        ...
        _ = services.AddEventNotificationHandler<TestEventNotificationHandler>()
                    .AddSingleton(p => new TestBroadcastingStrategy(observations, p, exception))
                    .AddSingleton(observations);
    ...
    {
        public async Task BroadcastEvent(IReadOnlyCollection<EventNotificationHandlerFn> eventNotificationHandlers,
                                         ...
                                         CancellationToken cancellationToken)
        {
            ...
            foreach (var handler in eventNotificationHandlers)
            {
