...
public sealed partial class AggregateSignalPublisherTests
{
    ...
    public async Task GivenConfiguredBroadcastingStrategy_WhenPublishingSignal_ConfiguredStrategyIsUsed(
        [Values(
            "default",
            "sequential",
            "sequentialWithConfig",
            "parallel",
            "parallelWithConfig",
            "custom")]
        string strategyType,
        [Values(true, false)] bool nestedPublisherThrowsException,
        [Values(true, false)] bool passPublishersAsEnumerable)
    {
        ...
        var handler = provider.GetRequiredService<ISignalPublishers>()
                              ...
                              .WithTransport(p =>
                              {
                                  ...
                                  Assert.That(innerTypes, Is.EqualTo(new[] { "test1", "test2", "test2" }));
                                  Assert.That(publisher.TransportTypeName, Is.EqualTo($"{ConquerorConstants.AggregateTransportName}[test1,test2,test2]"));

                                  return strategyType switch
                                  {
                                      "default" => publisher,
                                      "sequential" => publisher.WithSequentialBroadcastingStrategy(),
                                      "sequentialWithConfig" => publisher.WithSequentialBroadcastingStrategy(c => c.WithThrowOnFirstException()),
                                      "parallel" => publisher.WithParallelBroadcastingStrategy(),
                                      "parallelWithConfig" => publisher.WithParallelBroadcastingStrategy(c => c.WithMaxDegreeOfParallelism(null)),
                                      "custom" => publisher.WithBroadcastingStrategy(p.ServiceProvider.GetRequiredService<TestBroadcastingStrategy>()),
        ...
        {
            if (strategyType is "parallel" or "parallelWithConfig")
            {
                await Assert.ThatAsync(() => handler.Handle(signal), Throws.InstanceOf<AggregateException>().With.InnerException.SameAs(exception));
            }
            else
            {
                await Assert.ThatAsync(() => handler.Handle(signal), Throws.Exception.SameAs(exception));
            }
        }
        else
        {
            ...
            Assert.That(observations.ObservedPublisherExecutions, Is.EquivalentTo(new[]
            {
            ...
            {
                Assert.That(observations.ObservedCustomStrategyExecutions, Is.EqualTo(new[] { signal }));
            }
        }
    }
    ...
    private sealed class TestSignalPublisher1<TSignal>(
        ...
        Exception? exceptionToThrow = null) : ISignalPublisher<TSignal>
