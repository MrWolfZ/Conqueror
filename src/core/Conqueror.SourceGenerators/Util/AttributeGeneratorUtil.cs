...
public static class AttributeGeneratorUtil
{
        ...
        where TDescriptor : struct
    {
        var descriptors = context
            .SyntaxProvider.CreateSyntaxProvider((n, _) => HasAttribute(n), descriptorFactory)
        ...
        context.RegisterSourceOutput(
            ...
            (spc, descriptor) =>
            {
                if (descriptor is IHasDiagnostics d)
                {
        ...
        {
            var attributeLists =
                (node as ClassDeclarationSyntax)?.AttributeLists ?? (node as RecordDeclarationSyntax)?.AttributeLists;
            ...
            foreach (var attributeList in attributeLists)
            {
                ...
                {
                    if (
                        attribute.Name is SimpleNameSyntax ns
                        && ns.Identifier.Text.EndsWith(simpleAttributeNameSuffix, StringComparison.Ordinal)
                    )
                    {
                        return true;
                    }

                    if (attribute.Name.ToString().EndsWith(simpleAttributeNameSuffix, StringComparison.Ordinal))
                    {
                        return true;
                    }
                }
