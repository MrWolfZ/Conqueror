...
public sealed class StreamProducerServiceCollectionConfigurationTests
{
    ...
    public void GivenRegisteredProducerType_AddingIdenticalProducerOnlyKeepsOneRegistration()
    {
        ...
        Assert.That(services.Count(s => s.ServiceType == typeof(TestStreamProducer)), Is.EqualTo(expected: 1));
        Assert.That(
            services.Count(s => s.ServiceType == typeof(IStreamProducer<TestStreamingRequest, TestItem>)),
    ...
    public void GivenRegisteredProducerType_AddingProducerWithSameRequestAndItemTypesKeepsBothRegistrations()
    {
        ...
        Assert.That(services.Count(s => s.ServiceType == typeof(TestStreamProducer)), Is.EqualTo(expected: 1));
        Assert.That(services.Count(s => s.ServiceType == typeof(DuplicateTestStreamProducer)), Is.EqualTo(expected: 1));
        Assert.That(
            services.Count(s => s.ServiceType == typeof(IStreamProducer<TestStreamingRequest, TestItem>)),
            Is.EqualTo(expected: 1)
        );
    ...
    public void GivenRegisteredProducerType_AddingProducerWithSameRequestTypeAndDifferentResponseTypeThrowsInvalidOperationException()
    {
        ...
        _ = Assert.Throws<InvalidOperationException>(() =>
            services.AddConquerorStreamProducer<DuplicateTestStreamProducerWithDifferentResponseType>()
        );
    }
    ...
    public void GivenProducerTypeWithInstanceFactory_AddedProducerCanBeResolvedFromInterface()
    {
        var provider = new ServiceCollection()
            .AddConquerorStreamProducer(_ => new TestStreamProducer())
    ...
    {
        var provider = new ServiceCollection()
            .AddConquerorStreamProducer<TestStreamProducerWithMiddleware>()
    ...
    private sealed class DuplicateTestStreamProducerWithDifferentResponseType
        : IStreamProducer<TestStreamingRequest, TestItem2>
    {
    ...
    private sealed class TestStreamProducerWithMiddleware
