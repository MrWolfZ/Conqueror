...
public sealed class StreamingHandlerFunctionalityTests
{
    [Test]
    public async Task GivenRequest_HandlerReceivesRequest()
    {
        ...
        var handler = provider.GetRequiredService<IStreamingHandler<TestRequest, TestItem>>();
    ...
    {
        _ = Assert.Throws<ArgumentException>(() => new ServiceCollection().AddConquerorStreaming().AddTransient<TestStreamingHandlerWithoutValidInterfaces>().FinalizeConquerorRegistrations());
        _ = Assert.Throws<ArgumentException>(() => new ServiceCollection().AddConquerorStreaming().AddScoped<TestStreamingHandlerWithoutValidInterfaces>().FinalizeConquerorRegistrations());
        _ = Assert.Throws<ArgumentException>(() => new ServiceCollection().AddConquerorStreaming().AddSingleton<TestStreamingHandlerWithoutValidInterfaces>().FinalizeConquerorRegistrations());
    }
    ...
    private sealed class TestStreamingHandlerWithoutValidInterfaces : IStreamingHandler
    {
    }
