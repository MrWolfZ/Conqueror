...
public sealed class StreamingRequestIdTests
{
    ...
    public async Task GivenRequestExecution_RequestIdIsTheSameInProducerAndMiddlewareAndNestedClass()
    {
        ...
        Assert.That(observedRequestIds, Has.Count.EqualTo(expected: 3));
    ...
    public async Task GivenRequestExecution_RequestIdIsNotTheSameInNestedProducer()
    {
        ...
        Assert.That(observedRequestIds, Has.Count.EqualTo(expected: 2));
    ...
    )
    {
        ...
        _ = services.AddConquerorStreamProducer<TestStreamProducer>(p =>
            new(
                producerFn,
        ...
        _ = services.AddConquerorStreamProducer<NestedTestStreamProducer>(p =>
            new(nestedProducerFn, p.GetRequiredService<IConquerorContextAccessor>())
        );

        _ = services.AddConquerorStreamProducerMiddleware<TestStreamProducerMiddleware>(_ => new(middlewareFn));

        _ = services.AddConquerorStreamProducerMiddleware<OuterTestStreamProducerMiddleware>(_ =>
            new(outerMiddlewareFn)
        );
