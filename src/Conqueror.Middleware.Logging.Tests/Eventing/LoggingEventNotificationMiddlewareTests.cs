...
using static Conqueror.Middleware.Logging.Tests.Eventing.LoggingMiddlewareTestEventNotifications;
using Throws = NUnit.Framework.Throws;

namespace Conqueror.Middleware.Logging.Tests.Eventing;

[TestFixture]
public sealed class LoggingEventNotificationMiddlewareTests
{
    private const string TestEventNotificationId = "test-message-id";
        ...
        where THandler : class, IGeneratedEventNotificationHandler
    {
        await using var host = await LoggingMiddlewareTestHost.Create(
            services => services.RegisterEventNotificationType(testCase),
            logging =>
            {
        ...
        if (typeof(TEventNotification) == typeof(TestEventNotificationWithCustomTransport))
        {
            handler = handler.WithPipeline(p => ConfigureLoggingPipeline(p, testCase, addHooks: false))
                             .WithPublisher(b => new TestEventNotificationPublisher<TEventNotification>(b.UseInProcess(new TestEventNotificationBroadcastingStrategy(TestTransportName))));
        }
        ...
        where THandler : class, IGeneratedEventNotificationHandler
    {
        await using var host = await LoggingMiddlewareTestHost.Create(
            services =>
            {
                services.RegisterEventNotificationType(testCase);
                _ = services.Replace(ServiceDescriptor.Transient<IEventNotificationIdFactory, TestEventNotificationIdFactory>());
            },
            logging =>
            {
                _ = logging.AddTestLogger()
        ...
        [Values("pre", "post", "exception")] string hookThrowLocation)
    {
        ...
        if (hookThrowLocation is "exception")
        {
            var thrownException = Assert.ThrowsAsync<Exception>(() => handler.Handle(testCase.EventNotification, host.TestTimeoutToken));
        ...
        {
            await Assert.ThatAsync(() => handler.Handle(testCase.EventNotification, host.TestTimeoutToken), Throws.Nothing);
        }
    ...
    {
        await using var host = await LoggingMiddlewareTestHost.Create(
            services => services.AddEventNotificationHandler<TestEventNotificationHandler>(),
            logging =>
            {
                _ = logging.AddTestLogger(shouldTruncate: false)
    ...
    private sealed class TestEventNotificationIdFactory : IEventNotificationIdFactory
    {
        public string GenerateId() => TestEventNotificationId;
    }
}
