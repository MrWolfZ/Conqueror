...
[TestFixture]
public sealed class DataAuthorizationCommandMiddlewareTests : TestBase
{
    ...
    public async Task GivenSuccessfulAuthorizationCheck_WhenExecutedWithoutPrincipal_AllowsExecution()
    {
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(ConquerorAuthorizationResult.Success()));
    ...
    public async Task GivenFailedAuthorizationCheck_WhenExecutedWithoutPrincipal_AllowsExecution()
    {
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(ConquerorAuthorizationResult.Failure("test")));
    ...
    [Test]
    public void GivenFailedAuthorizationCheck_WhenExecutedWithAuthenticatedPrincipal_ThrowsDataAuthorizationFailedException()
    {
        handlerFn = _ =>
        {
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(result));

        using var d = AuthenticationContext.SetCurrentPrincipal(new(new ClaimsIdentity("test")));

        var thrownException = Assert.ThrowsAsync<ConquerorDataAuthorizationFailedException>(() => Handler.ExecuteCommand(new()));
    ...
    public async Task GivenMultipleFailedAuthorizationChecks_WhenExecutedWithoutPrincipal_AllowsExecution()
    {
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(ConquerorAuthorizationResult.Failure("test 1")))
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(ConquerorAuthorizationResult.Failure("test 2")));
    ...
    [Test]
    public void GivenMultipleFailedAuthorizationChecks_WhenExecutedWithAuthenticatedPrincipal_ThrowsDataAuthorizationFailedException()
    {
        handlerFn = _ =>
        {
            Assert.Fail("handler should not be executed");
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(result1))
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(result2));
    ...
    [Test]
    public void GivenMixedSuccessfulAndFailedAuthorizationChecks_WhenExecutedWithAuthenticatedPrincipal_ThrowsDataAuthorizationFailedException()
    {
        handlerFn = _ =>
        {
            Assert.Fail("handler should not be executed");
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(successResult))
                                                .AddDataAuthorizationCheck<TestCommand>((_, _) => Task.FromResult(failureResult));
    ...
    [Test]
    public async Task GivenRemovedDataAuthorizationMiddleware_WhenExecutedWithoutPrincipal_AllowsExecution()
    {
        var testCommand = new TestCommand();
        var expectedResponse = new TestCommandResponse();
    ...
    [Test]
    public async Task GivenRemovedDataAuthorizationMiddleware_WhenExecutedWithUnauthenticatedPrincipal_AllowsExecution()
    {
        var testCommand = new TestCommand();
        var expectedResponse = new TestCommandResponse();
    ...
    [Test]
    public async Task GivenRemovedDataAuthorizationMiddleware_WhenExecutedWithAuthenticatedPrincipal_AllowsExecution()
    {
        var testCommand = new TestCommand();
        var expectedResponse = new TestCommandResponse();
    ...
    public async Task GivenAuthorizationCheckForBaseCommandType_WhenExecutedWithAuthenticatedPrincipal_AllowsExecution()
    {
        ...
        configurePipeline = pipeline => pipeline.UseDataAuthorization()
                                                .AddDataAuthorizationCheck<BaseTestCommand>((_, _) => Task.FromResult(ConquerorAuthorizationResult.Success()));
